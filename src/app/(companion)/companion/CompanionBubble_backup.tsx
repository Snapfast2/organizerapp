...
export default function CompanionBubble() {
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    api?.companion?.setClickThrough?.(true);
  ...
  const handleSaveToHub = async () => {
    ...
    setIsMigrating(true);
    try {
      // Step 1: Request the active project path via script (focus steal expected here)
      ...
      // Step 2: Check if already in Hub
      const inHub = await api?.companion?.isProjectInHub?.(realProj);
      if (inHub) {
        setIsMigrating(false);
        return;
  ...
  const handleShare = async (e: React.MouseEvent, targetPath: string) => {
    e.stopPropagation();
    try {
      setSharingPath(targetPath);
      // 1. Compress
      const compRes = await fetch('http://127.0.0.1:3000/api/fs/compress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // 2. Upload
      const shareRes = await fetch('http://127.0.0.1:3000/api/fs/share', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      navigator.clipboard.writeText(`¡Hola! Aquí está el archivo: ${shareData.url}`);
      
      // Use standard notification for success
      new Notification('MooMotion Cloud', { body: 'Enlace copiado al portapapeles' });
      
    } catch (e: any) {
      new Notification('MooMotion Error', { body: e.message });
    } finally {
      setSharingPath(null);
    }
  };
          ...
          <div className={styles.body}>
            ...
            {isUntracked && (
              <div style={{
                ...
                <div style={{ fontSize: 11.5, fontWeight: 600, color: '#e5e7eb' }}>
                  ¿Proyecto externo?
                </div>
                <button 
                ...
                {recents.slice(0, 4).map((f, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '4px', width: '100%' }}>
                    <button
                      ...
                      style={{ WebkitAppRegion: 'no-drag', flex: 1, overflow: 'hidden' } as React.CSSProperties}
                      title={f.path}
                    >
                    ...
                    {/* SHARE FEATURE — pausado
                    <button 
                      onClick={(e) => handleShare(e, f.path)}
                      style={{ WebkitAppRegion: 'no-drag', padding: '4px', background: 'transparent', border: 'none', cursor: 'pointer', borderRadius: '4px' }}
                      title="Comprimir y Compartir"
                    >
                      {sharingPath === f.path ? <CloudUpload size={12} color="#60a5fa" className="spin-animation" /> : <CloudUpload size={12} color="#888" />}
                    </button>
                    */}
                  </div>
                ))}
